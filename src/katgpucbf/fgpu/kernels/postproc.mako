 ...
 *   spectra_per_heap_out complete spectra.
 *
 * A note on stride length:
 * `out` is a multi-dimensional array of shape (heaps x channels x spectra_per_heap_out). If
...
KERNEL void postproc(
    ...
    int spectra_per_heap_out,                 // Number of spectra per output heap.
    ...
    float quant_gain)                         // Scale factor for quantiser.
{
    ...
    <%transpose:transpose_load coords="coords" block="${block}" vtx="${vtx}" vty="${vty}" args="r, c, lr, lc">
        ...
        int spectrum = z * spectra_per_heap_out + ${r};
