...
class PostprocTemplate:
    ...
    def instantiate(
        self, command_queue: AbstractCommandQueue, spectra: int, spectra_per_heap_out: int, channels: int
    ) -> "Postproc":
        """Generate a :class:`Postproc` object based on this template."""
        return Postproc(self, command_queue, spectra, spectra_per_heap_out, channels)
...
class Postproc(accel.Operation):
    ...
    **out** : (spectra // spectra_per_heap_out, channels, spectra_per_heap_out, 2, 2), int8
    ...
    def __init__(
        ...
        if spectra % spectra_per_heap_out != 0:
            raise ValueError("spectra must be a multiple of spectra_per_heap_out")
        ...
        if spectra_per_heap_out % block_y != 0:
            raise ValueError(f"spectra_per_heap_out must be a multiple of {block_y}")
        ...
        self.slots["out"] = accel.IOSlot(
            (spectra // spectra_per_heap_out, channels, spectra_per_heap_out, _2, _2), np.int8
        )
    ...
    def _run(self) -> None:
        ...
        groups_y = self.spectra_per_heap_out // block_y
        groups_z = self.spectra // self.spectra_per_heap_out
        out = self.buffer("out")
        in0 = self.buffer("in0")
        in1 = self.buffer("in1")
        self.command_queue.enqueue_kernel(
            ...
            [
                ...
                np.int32(self.spectra_per_heap_out),  # spectra_per_heap_out
