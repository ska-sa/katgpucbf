"""xbgpu receiver example script.

This script demonstrates how to configure a xbgpu receiver object to receive data from an fsim or MeerKAT
channelised voltage stream (or more simply an X-Engine input stream).

It also shows how a chunk can be received asynchronously and how to pass a used chunk back to the xbgpu receiver.
"""
import argparse
import asyncio
import logging
...
parser = argparse.ArgumentParser(description="Simple example demonstrating how to use xbgpu receiver software.")
...
# katgpucbf.xbgpu._katxbgpu.recv.Stream object.
thread_affinity = 2
n_ants = 64
...
if use_file_monitor:
    monitor: katgpucbf.xbgpu.monitor.Monitor = katgpucbf.xbgpu.monitor.FileMonitor("temp_file.log")
else:
...
async def get_chunks():
    """Receive and process completed chunks from the xbgpu receiver."""
