...
def make_figure(x_range, y_range):
    return figure(
        x_range=x_range, y_range=y_range, tooltips=[("x", "$x"), ("y", "$y"), ("mag", "@mag"), ("phase", "@phase")]
    )
...
for pol in range(2):
    ...
    pmag.image(image="mag", x="x", y="y", dw="dw", dh="dh", color_mapper=mapper, source=source)

    pphase = make_figure(x_range, y_range)
    mapper = LinearColorMapper(colorcet.colorwheel, low=-math.pi, high=math.pi)
    pphase.image(image="phase", x="x", y="y", dw="dw", dh="dh", color_mapper=mapper, source=source)
