...
def test_recv_simple(event_loop, num_ants, num_spectra_per_heap_in, num_channels):
    ...
    for _ in range(5):
        heaps = create_heaps(
            timestamp_step * heap_index, heap_index, n_ants, n_channels_per_stream, n_spectra_per_heap_in, n_pols, ig
        )
        ...
        async for chunk in async_ringbuffer:
            ...
            for heap_index in range(heaps_per_fengine_per_chunk):
                for ant_index in range(n_ants):
                    ...
                    fengine_start_index = (
                        (heap_index * n_ants + ant_index) * n_channels_per_stream * n_spectra_per_heap_in * n_pols
                    )
                    fengine_stop_index = fengine_start_index + n_channels_per_stream * n_spectra_per_heap_in * n_pols
