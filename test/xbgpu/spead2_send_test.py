...
def test_send_simple(event_loop, num_ants, num_channels):
    ...
    # 3.2 Create katxgpu.xsend.XEngineSPEADInprocSend that will wrap a SPEAD2 send stream.
        ...
        while num_received < heaps_to_send:
            ...
            for item in items.values():
                ...
                if item.id == 0x1800:
                    ...
                    assert (
                        item.value.dtype == np.int64
                    ), f"xeng_raw dtype is {(item.value.dtype)}, dtype of int64 expected."
