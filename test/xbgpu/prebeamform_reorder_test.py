...
from katxgpu.prebeamform_reorder import PreBeamformReorderTemplate
...
def test_prebeamform_reorder_parametrised(num_ants, num_channels, num_samples_per_channel):
    """
    ...
    # bufSamples_host.dtype = np.int8
    total_size = n_batches*num_ants*n_channels_per_stream*num_samples_per_channel*2
    bufSamples_host[:] = np.arange(total_size).reshape(n_batches,num_ants,n_channels_per_stream,num_samples_per_channel,2)
...
ants = 64
chan = 32768
samples_per_chan = 256
print(f'Stop')

test = test_prebeamform_reorder_parametrised(ants, chan, samples_per_chan)
b = 1