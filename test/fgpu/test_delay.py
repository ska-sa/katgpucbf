...
from katfgpu.delay import LinearDelayModel, MultiDelayModel


@pytest.fixture
def linear() -> LinearDelayModel:
    return LinearDelayModel(12345, 100.0, 0.25)
...
def test_linear_invert(linear: LinearDelayModel) -> None:
    assert linear.invert(12445) == (12345, 0.0)
    assert linear.invert(13001) == pytest.approx((12790, 0.2))
    assert linear.invert(12999) == pytest.approx((12788, -0.2))


def test_linear_invert_range(linear: LinearDelayModel) -> None:
    time, residual = linear.invert_range(12999, 13005, 1)
    ...
    time, residual = linear.invert_range(13000, 14000, 100)
